...
from sqlalchemy import Column, String, BIGINT, INT, Enum

from app.constants.ship_type_enum import ShipTypeEnum
...
class Ship(Base):
    ...
    ship_type = Column(Enum(ShipTypeEnum), comment="船舶类型")
    ship_capacity = Column(INT, default=0, comment="载重吨")
    status = Column(INT, comment="当前状态")
